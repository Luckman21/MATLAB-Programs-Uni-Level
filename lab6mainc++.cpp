...
int global_delay_val1 = 1000;        // vary this to change the pulse's delay.
int global_delay_val3 = 3000;        // vary this to change the pulse's delay.
...
int main(int argc, char** argv) {
    ...
    while(1) 
    {
        ...
        delay(global_delay_val1);         // 1000: roughly Delay for roughly 1 second with 187.5 kHz system clock
    	LATA = 0x0001;		// Set Pin 0 on PORT A to logic level "high" (3v).    

        // Turn LED off (takes roughly 1 sec)
        delay(global_delay_val3);         // 1000: roughly Delay for roughly 1 second with 187.5 kHz system clock
